...
@section('content')
<div class="container">
    <form id="editor_form" action="/user/posts/{{$post->id}}/update" method="POST">
        @csrf
        @method("PUT")
        <div class="d-flex" style="@error('title') border-color:red; @enderror">
            <div class="input-group input-group-lg">
                ...
                <input name="title" id="input_title" type="text" value="{{$post->title}}" class="form-control"
        ...
        <div id="tag_list" name="tag_list">
        </div>
        <button name="submit_button" class="btn btn-primary mt-2" onsubmit="onSave()" type="submit">Update</button>
    </form>
...
<script>
    ...
    //adding current tags selected for post that is being edited.
    const post_tags = {!! json_encode($post_tags)!!};

    if (typeof post_tags != "undefined" && post_tags.length > 0) {
        post_tags.map(({ tag }) => {
            let index = getCounter() + 1;
            $('#tag_list').append(`<a id='tag_${index}' onclick="onClick(event)" name='tag_${index}' href='#editor' class='badge badge-info ml-2 tags selected'>${tag}</a>`)
            $("#editor_form").append(`<input name="tags[]" type="hidden" value="${tag}" element_id="tag_${index}">`)
            setCounter(index);
        });
    }
    ...
    function getCounter() {
        return parseInt($("#custom_tag_counter").attr("value"), 10);
    }
